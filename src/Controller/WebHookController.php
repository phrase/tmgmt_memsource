...
class WebHookController extends ControllerBase {
  ...
  public function callback(Request $request) {
    ...
    if ($config->get('debug')) {
      \Drupal::logger('tmgmt_memsource')->debug('Request received %request.', ['%request' => $request]);
      \Drupal::logger('tmgmt_memsource')->debug('Request payload: ' . $request->getContent());
    }
    ...
    foreach ($jobParts as $jobPart) {
      ...
      if (isset($project_id) && isset($job_part_id) && isset($status)) {
        ...
        if (empty($remotes)) {
          \Drupal::logger('tmgmt_memsource')->warning('Project %id not found.', ['%id' => $project_id]);
        ...
        if (!$remote) {
          \Drupal::logger('tmgmt_memsource')->warning('File %id not found.', ['%id' => $job_part_id]);
          return new Response(new FormattableMarkup('File %id not found.', ['%id' => $job_part_id]), 404);
        }
        if ($workflow_level != $last_workflow_level) {
          \Drupal::logger('tmgmt_memsource')->warning('Workflow level %workflow_level is not the last workflow level %last_workflow_level: project %project_id, job part %job_part_id',
            ['%workflow_level' => $workflow_level, '%last_workflow_level' => $last_workflow_level, '%project_id' => $project_id, '%job_part_id' => $job_part_id]);
        ...
        if (!$translator_plugin->remoteTranslationCompleted($status)) {
          \Drupal::logger('tmgmt_memsource')->warning('Invalid job part status %status: project %project_id, job part %job_part_id',
            ['%status' => $status, '%project_id' => $project_id, '%job_part_id' => $job_part_id]);
          return new Response(new FormattableMarkup('Project %id not found.', ['%id' => $project_id]), 400);
        }
